...
\renewcommand{\maketitle}{
	\begin{center}
		\huge\bfseries{Properties of Motion\\ in Central Force}
	\end{center}
	\bigskip
}
...
\indent In classical mechanics, a central force on a particle is a force that is directed towards or away from a point called center of force. We will add another condition for convenience. The magnitude of the force only depends on the distance between that point and the particle.
\[ \vec{F}(\r) = F(r) \ru \labelthis{eq:def central force} \]

Where,
\begin{itemize}
	\item $\r$ is the vector pointing from center of force to the particle.
...
Central force is a conservative force. This follows any of the following facts.
...
\indent Let us imagine a particle starting from the position $\r_0$ with velocity $\v_0$. After a small time $\Delta t$ the particle moves to $\r_1 = \r_0 + \v_0 \, \Delta t$ which lies in the plane created by the span of $\r_0$ and $\v_0$ (let's call this RV plane). At $\r_1$ the velocity changes to $\v_1 = \v_0 + f(r_0) \, {\r_0}/{r_0} \, \Delta t$ which also lies in RV plane. Thus $ \r_2, \r_3, \dots, \r_n$ and $ \v_2, \v_3, \dots, \v_n$ also lies in RV plane. This results in the whole motion being constrained within a 2D flat plane.
...
\[ \ddt{\vec{L}} = \ddt{} (\r \times m \v) = \r \times \vec{F} = \mathbf{0} \]
...
\indent Let $\theta$ be the angle $\r$ makes with the $+x$ axis (counter clockwise is positive). Newton's third law has second derivative of $\r$ in cartesian coordinates. So, we should start with evaluating the second derivatives of $r$ in terms of $\r$ and see what we can do with it.
...
\noindent But $v_\bot = \dot{\theta} r$. Where $\dot{\theta}$ is the limit of the ratio of the angle between two consecutive $\r$ to the time difference as the time difference goes to $0$.  For the polar coordinate system $\dot{\theta}$ is simply the time derivative of $\theta$.  Thus,
\[ \ddtII{r} = \dot{\theta}^2 \, r + f(r) \labelthis{eq:eom} \]

\subsection{Using lagrangian mechanics}\label{eom lagrangian}
\indent Let $T$ be the kinetic energy and $U$ be the potential energy of the particle experiencing the central force $\vec{F}(\r)$.
\begin{align}
...
\[ L = T - U = \frac{1}{2} \, m \left( \dot{r}^2 + r^2 \dot{\theta}^2 \right) + m \int f(r) \, dr 
																					\labelthis{eq:lagrangian} \]

The Euler-Lagrange equations state that,
\[	\ddt{} \frac{\partial L}{\partial \dot{r}} = \frac{\partial L}{\partial r} \hspace{3em}
\ddt{} \frac{\partial L}{\partial \dot{\theta}} = \frac{\partial L}{\partial \theta} \]

Substituting for $L$ in the above equations we get,
\begin{align*}
	&\ddtII{r} = \dot{\theta}^2 r + f(r) \\
	&\ddt{} (m r^2 \dot{\theta}) = 0
\end{align*}

The first equation is the same as~\eqref{eq:eom} and the second one is just stating the conservation of angular momentum.
...
Angular momentum is polar coordinate is $L = m r^2 \dot{\theta}$. Then,
\[ \dot{\theta} = \frac{L}{m r^2} = \frac{h}{r^2} \labelthis{eq:ddttheta} \]

The angular momentum is conserved for central force. So differentiating both sides,
...
\indent This is one messy equation. But differential equations are messy. Maybe we can get a simpler equation if we use some other variables instead of $r$ and $\theta$?
Let us take a hint from the gravitaional field. We know that the planets follow an elliptical path.  The equation of a elipse in Polar coordinates is given by,
\[ r = \frac{k}{1 + e \cos{\theta}} \]

From this we can observe that $1/r$ and $\cos{\theta}$ has a linear relationship. These kind of relationships are possible as a solution of linear differential equations. Thus let us choose our new variable $u = 1/r$. Then,
\begin{align*}
...
The parts inside the parenthesis is exactly ${d^2 u}/{{d\theta}^2}$. After rearranging some terms we arrive at,
